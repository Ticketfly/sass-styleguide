# Ticketfly Sass Styleguide

_A super simple styleguide for your styles! (Mostly based on AirBnB's CSS Styleguide)_
...
```
ember install Ticketfly/sass-styleguide
```

### Updating

The addon adds `.sass-lint.yml` to the root directory so text editors such as
Atom and Sublime can lint on the fly to show you errors and help you write
correct Sass easily and quickly. When you want to update the styleguide you'll
have to rerun the blueprint to regenerate the file and add it.

To update run:

```
ember generate sass-styleguide
```
...
* Use full 6 digit lowercased hex values
* Use leading zeros for decimal properties
* No trailing zeros on fields
* Use shorthand for collapsible fields (margin, padding, etc.)
* Drop units from zero values
* Prefer `none` to `0` on relevant fields (border)
...
* Follow the [Concentric CSS property ordering](https://github.com/sasstools/sass-lint/blob/develop/lib/config/property-sort-orders/concentric.yml)
...
In certain cases ID selectors may be necessary, particularly when dealing with legacy apps. Overriding existing styles
on elements which use an ID selector, for instance. This should be avoided if possible.

For more on this subject, read [CSS Wizardry's article](http://csswizardry.com/2014/07/hacks-for-dealing-with-specificity/) on dealing with specificity.

### JavaScript hooks

Avoid using classes for javascript hooks in most circumstances. In Ember, component javascript hooks should suffice for most use cases.
...
5. Color Variables
    ...
    Color literals can become very verbose and make CSS code difficult to read. Do not use them directly in your Scss. Instead, make variables for your values and substitute those in.
...
Note: When using `ember-component-css`, the first level of the component does not
